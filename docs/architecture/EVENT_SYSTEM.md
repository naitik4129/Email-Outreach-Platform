...
## Message-level tracking events (opens and bounces)

`recipient_outcomes` records what happened to an *enrollment*. Open and bounce analytics need what happened to a single outbound *message*, so `message_events` (migration 0030) holds at most one row per `(workspace, message, kind)` with `kind` `OPENED` or `BOUNCED`. Repeats never add rows: they increment `occurrence_count`/`last_occurred_at`, so retries, prefetching and webhook redelivery cannot inflate distinct-message metrics. Lead, campaign, sequence, step and mailbox are reached through the message.

- **Opens.** Campaign emails carry a 1×1 pixel added at send time (never at render time, which would change the frozen content digest) when `OPEN_TRACKING_ENABLED`, `TRACKING_BASE_URL` and `TRACKING_SIGNING_KEY` are all set. The public endpoint `GET /api/v1/t/o/{token}.gif` verifies an HMAC token (workspace + message, no personal data) without a database read, upserts the row, and always answers the same transparent GIF with `Cache-Control: no-store`; invalid, forged, unknown or failing requests are indistinguishable and write nothing. `HEAD` is not an open. Opens are an *estimate*: mail-privacy proxies and link scanners can inflate or hide them.
- **Bounces.** One service (`events/bounce_service.py`) records every bounce, whether it arrives as a signed webhook or as a delivery-status notification found by mailbox sync. It upserts the message's `BOUNCED` row (hard outranks soft; duplicates and late notifications only count), and for HARD additionally suppresses the address, stops active enrollments, records the outcome even when the enrollment already finished, and cancels unsent follow-ups. SOFT/UNKNOWN are evidence only. Hard/soft follows the reported RFC 3463 class (5.x.x permanent, 4.x.x transient, 5.2.2 mailbox-full treated as soft) or explicit provider verdict; nothing is invented when the provider says nothing.
- **Metric definitions** (one SQL definition, `AnalyticsRepository._message_event_counts`, used by campaign, step, workspace and deliverability views): *sent* = SENT messages; *bounced* = distinct sent messages with a BOUNCED event; *delivered (estimated)* = sent − bounced (no provider delivery receipt exists, so it is never called "delivered" without the estimate label); *opened* = distinct delivered messages with an OPENED event; *bounce rate* = bounced ÷ sent; *open rate* = opened ÷ delivered (estimated). A "replied" lead is a genuine human reply only.
