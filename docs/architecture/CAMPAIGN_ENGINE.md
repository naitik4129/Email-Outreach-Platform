...
- **Hyper-personalized campaigns (proposed, [ADR-0011](../adr/0011-hyper-personalized-campaign-type.md))** keep this engine and every state machine. `campaigns.campaign_type=HYPER_PERSONALIZED` changes only how a message's content snapshot is produced: the message is inserted PLANNED with an intended due time and a `message_generations` row, and becomes SCHEDULED when a validated snapshot is written (`renderer_version=2`, same digest formula) roughly one lead-time before it is due. Activation freezes the objective, reference templates and prompt version, not per-lead output; the per-lead snapshot is frozen before QUEUED as usual. Insufficient context renders the reference template (`renderer_version=1`); a failure after bounded attempts marks the message FAILED and it is never sent. Eligibility is rechecked in the finalizing transaction and by the unchanged send gates. See also [ADR-0012](../adr/0012-llm-port-data-handling-and-validation.md) and [ADR-0013](../adr/0013-research-sources-and-outbound-fetch.md).
