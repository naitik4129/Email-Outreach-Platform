...
|---|---|---|
| Gmail | Requires the `gmail.readonly` scope. Initial scan of `INBOX` limited to `REPLY_SYNC_INITIAL_HORIZON_DAYS` (default 30), then a `history.list` cursor filtered to `INBOX`. The mailbox position is captured *before* the scan starts and carried in a tagged cursor (`full`/`hist`), so a page token is never used against the wrong API. Polling only; no Pub/Sub watch is registered. | Invalid/out-of-range history cursor requires full resync. A message that cannot be read aborts the page (the cursor does not advance); only "no longer exists" is skipped. Own SENT/DRAFT copies are ignored. |
| Microsoft | Requires `Mail.ReadWrite` (also used to create the draft that reveals the Message-ID at send time). Inbox delta with an initial `receivedDateTime` horizon filter (falls back to plain delta if Graph rejects the filter); In-Reply-To/References are read per message because delta does not reliably return headers. Polling only. | Opaque continuation links are validated against the Graph origin. Match by RFC header when present, otherwise by `conversationId` + sender. |
| SMTP | Reply sync over **IMAP** for mailboxes saved with IMAP settings (993 implicit TLS / 143 STARTTLS, SSRF-pinned like SMTP). `INBOX` read-only, cursor `{uidvalidity, last_uid}`, initial horizon as above. The provider message id is the RFC Message-ID (fallback `imap:{uidvalidity}:{uid}`) so a UIDVALIDITY reset cannot store a message twice. | A mailbox without IMAP settings is parked (`IMAP_NOT_CONFIGURED`) and shown as such; SMTP alone never claims reply support. UIDVALIDITY change → resync. |
...
## How a mailbox becomes synchronized

Every scheduler tick first makes sure each CONNECTED mailbox has a `mailbox_sync_states` row (`ReplySyncService.ensure_sync_states`, idempotent), then dispatches due rows to `worker-sync`. Without that row nothing is ever dispatched, which is why replies were previously never detected. Before any provider call the sync checks that the stored connection can actually read mail (`mailboxes/reply_capability.py`: granted read scope for Gmail/Microsoft, IMAP settings for SMTP). A mailbox that cannot is parked as `UNAVAILABLE` with a reason (`RECONNECT_REQUIRED`, `IMAP_NOT_CONFIGURED`), mirrored to `mailboxes.sync_state`, and re-armed automatically when a new connection generation is created (reconnect, or changed IMAP settings). Failures back off exponentially (`REPLY_SYNC_MAX_BACKOFF_SECONDS`, `failure_count`) and are logged with mailbox, provider and retry time but never message content. A resync safety hold is released once the full resync completes.
...
Outbound identifiers: every campaign send records `messages.rfc_message_id` — the Message-ID the recipient's reply will quote, as reported by the provider (Gmail is read back after sending, Graph returns it from the draft created before sending, SMTP uses the header we set) — and the provider thread/conversation id on `message_attempts.provider_thread_ref`. Message-IDs are compared without angle brackets and case-insensitively.
...
Approved policy (2026-09-26): only a recognized **human** reply stops the matched enrollment and counts as Replied. Out-of-office and other automatic responses are stored, linked to the outbound email and shown, but neither stop the sequence nor count as Replied. Delivery-status notifications are recognized during sync (multipart/report, MAILER-DAEMON/postmaster senders, Exchange NDRs), parsed for the failed recipient, RFC 3463 status class and the original Message-ID, and routed to the shared bounce service (see [EVENT_SYSTEM](EVENT_SYSTEM.md)); they are never stored as replies. A DSN is attached to the original email by Message-ID, then the returned headers' references, and only if it carried no identifier at all by the recipient's most recent sent email (flagged as weaker evidence). Identifiers that match nothing are treated as contradicting evidence, and a DSN's claimed recipient can never suppress an address on its own. Suspicious/ambiguous evidence imposes a candidate-specific or mailbox hold for review rather than false attribution. Separate unsubscribe/complaint signals call suppression for workspace-wide prohibition. Reply alone stops the matched enrollment, not every campaign for that lead. Changing that product scope requires approval.
...
Approve exact reply recognition confidence rules, sync freshness SLO and the single-step send-only exception. (Decided 2026-09-26: OOO/automatic responses do not stop sequences; initial horizon defaults to 30 days; SMTP receive compatibility is provided by IMAP.) Replies from a different address or alias remain UNRESOLVED until an explicit alias policy exists. Recommended fail-closed follow-up behavior may reduce sending availability; it avoids pretending reply stopping works on an unsynchronized mailbox. Related: [events](EVENT_SYSTEM.md), [suppression](SUPPRESSION.md), [providers](PROVIDER_ARCHITECTURE.md), [database](../database/DATABASE.md).
