...
import { completeMicrosoftOAuth } from "@/lib/mailboxes-api";
...
function MicrosoftCallbackInner() {
  ...
  useEffect(() => {
    ...
    if (urlError) {
      ...
      setStatus("error");
      setErrorMessage(`Microsoft authentication error: ${urlError}`);
      return;
    }
    ...
    async function finishOAuth() {
      try {
        const res = await completeMicrosoftOAuth(activeWorkspaceId!, {
          code: code!,
          state: state!,
        });
        ...
        if (err instanceof ApiError) {
          ...
          setErrorMessage("Failed to complete Microsoft connection. Please try again.");
        }
      }
    }
